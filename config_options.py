...
def _get_GTEP_config():
    ...
    CONFIG.declare("region", ConfigValue(
        default="ERCOT",
        domain=In(["ERCOT"]),
    ...
    CONFIG.declare("time_horizon", ConfigValue(
        default=5,
    ...
    CONFIG.declare("num_repn_days", ConfigValue(
        default=10,
        domain=PositiveInt,
    ...
    CONFIG.declare("formulation", ConfigValue(
        default="improved",
    ...
    CONFIG.declare("algo", ConfigValue(
        default="benders",
    ...
    CONFIG.declare("clustering_algorithm", ConfigValue(
        default="kmeans",
    ...
    CONFIG.declare("benders_mip_gap", ConfigValue(
        default=5e-3,
        domain=PositiveFloat,
    ...
    CONFIG.declare("nested_benders_iter_limit", ConfigValue(
        default=100,
        domain=PositiveInt,
    ...
    CONFIG.declare("nested_benders_relax_integrality_forward", ConfigValue(
        default=1,
        domain=In([0,1]),
    ...
    CONFIG.declare('write_gtep_results', ConfigValue(
        default=True,
...
def check_config(config):
    ...
    if (config.repn_day_method == "cost" or config.extreme_day_method != "load_shedding_cost")and config.time_horizon != 5:
        config.logger.error('investment cost for %s years has not been calculated. switch to input-based clustering' % (config.time_horizon,))
        config.repn_day_method = "input"
        config.extreme_day_method = "load_shedding_cost"
    if config.region != "ERCOT":
        config.logger.error('we do not support %s region, switch to ERCOT' % (config.region,))
