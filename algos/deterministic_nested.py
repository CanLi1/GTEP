...
def nested_benders_solve(config, InvestData, OperationalData):
	...
	for iter_ in range(1, max_iter+1):
		...
		stage = 1
		n = 'O'
		config.logger.info("Forward Pass: Stage %s Current Node %s" % (stage, n,))
		op = 0

		ngo_rn, ngo_th, nso, nte, cost = forward_pass(m.Bl[stage], rn_r, th_r, j_r, l_new, t_per_stage[stage], relax_integrality, config)
		...
		cost_UB[iter_] = min(cost_forward[kk] for kk in range(1, max_iter+1) if kk <= iter_)
			...
			if stage != n_stages:
				for n in n_stage[stage]:
					for cn in children_node[n]:
						for op in [0]:
							config.logger.info("Children Node %s of stage %s op scenario %s" % (cn,  stage + 1,  op,))
							...
							mltp_rn, mltp_th, mltp_s, mltp_t, cost = backward_pass(stage + 1, m.Bl[stage + 1], n_stages,
																		   rn_r, th_r, j_r, l_new, config)

							cost_backward[stage + 1, cn, op, iter_] = cost
							config.logger.info('cost %s %s %s %s' % (stage + 1, cn, op, cost_backward[stage + 1, cn, op, iter_],))
