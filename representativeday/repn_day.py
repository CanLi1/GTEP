...
day_end = 366
with open(
    "repn_results/5yearsinvestment_NETL_no_reserve"
    + str(day_start)
    + "-"
    + str(day_end)
    + ".csv",
    ...
    for day in range(day_start, day_end):
        ...
        (
            ...
            prob,
            sc_nodes,
        ) = create_scenario_tree(stages, scenarios, single_prob)
        readData_single.read_data(filepath, curPath, stages, n_stage, t_per_stage, day)
        sc_headers = list(sc_nodes.keys())
        max_iter = 100
        ...
        for t in m.t:
            ...
            for l in m.l_new:
                ...
                key = (
                    readData_single.tielines[l - 1]["Near Area Name"]
                    + "_"
                    + readData_single.tielines[l - 1]["Far Area Name"]
                    + "["
                    + str(t)
                    + "]"
                )
