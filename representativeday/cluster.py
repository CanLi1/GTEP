...
def load_cost_data(config):
    ...
    investment = pd.read_csv(
        "5yearsinvestment_NETL_no_reserve1-366_MIP.csv", index_col=0, header=0
    ).iloc[:, :]
    data = investment.obj
    ...
    for line in lines:
        if (
            sum(
                investment.loc[:, "cost_" + line + "[" + str(t) + "]"].sum()
                for t in stages
            )
            > 0.1
        ):
    ...
    for series in input_data:
        if i == 0:
            ...
            normalized_data = pd.concat(
                [
                    normalized_data,
                    (series - series.mean()).divide(np.sqrt(series.var())),
                ],
                axis=1,
            )
        i += 1
...
def select_extreme_days(
    ...
    if (
        method == "load_shedding_cost"
    ):  # select the infeasible days with largest load shedding cost
        days = []  # days selected
        if len(infeasible_days) <= n:
            days = infeasible_days
        else:
