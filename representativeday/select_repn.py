...
def select_repn_days(config):
    if config.repn_day_method == "cost":
        ...
        clustering_result = run_cluster(
            data=data,
            config=config,
            method=config.clustering_algorithm,
            n_clusters=config.num_repn_days,
        )
    ...
    config.logger.info(
        "Perform %s to select %s representative days based on %s data"
        % (
            config.clustering_algorithm,
            config.num_repn_days,
            config.repn_day_method,
        )
    )
