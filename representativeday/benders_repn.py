...
outputfile = (
    ...
    + extreme_day_method
    + "_"
    + str(initial_cluster_num)
    + "days_5years_mediumtax_no_reserve.csv"
)
...
if method == "cost":
    ...
    result = run_cluster(
        data=data, method=clustering_method, n_clusters=initial_cluster_num
    )
    initial_cluster_result = result
elif method == "input":
    data = load_input_data()
    initial_cluster_result = run_cluster(
        data=data, method=clustering_method, n_clusters=initial_cluster_num
    )
print(outputfile)
iter_ = 1
...
while True:
    ...
    if clustering_method == "kmeans" or clustering_method == "kmeans_exact":
        readData_det.read_data(
            ...
            len(np.unique(cluster_results["labels"])),
        )
    elif clustering_method == "kmedoid_exact" or clustering_method == "kmedoid":
        readData_det.read_data(
            filepath,
    ...
    # create blocks
    m = b.create_model(
        n_stages, time_periods, t_per_stage, max_iter, formulation, readData_det
    )
    ...
    if iter_ == 1:
        ...
        write_GTEP_results(
            m, outputfile, opt, readData_det, t_per_stage, results, mode="a"
        )
    ...
    new_model = b.create_model(
        n_stages, time_periods, t_per_stage, max_iter, formulation, readData_single
    )
    ...
    for day in range(1, 366):
        operating_cost[day] = eval_investment_single_day(
            new_model, day, n_stages, readData_det, t_per_stage, load_shedding
        )
