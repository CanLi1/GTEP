    ...
    for i in ref_model.stages:
        for v1 in ref_model.Bl[i].component_objects(Var):
            for v2 in new_model.Bl[i].component_objects(Var):
                if (
                    v1.getname() in investment_vars
                    and v2.getname() in investment_vars
                    and v1.getname() == v2.getname()
                ):
                    for index in v1:
...
def eval_investment_single_day(new_model, day, config, load_shedding=True):
    ...
    for i in range(1, n_stages + 1):
        ...
        if (results.solver.status == SolverStatus.ok) and (
            ...
            raise Exception(
                "the problem at a given time is not solved to optimality termination_condition is ",
    ...
    if total_operating_cost >= 1e9:
        ...
        if load_shedding:
            ...
            for i in range(1, n_stages + 1):
                ...
                operating_related_cost["load_shedding_cost"] += new_model.Bl[
                    i
                ].load_shedding_cost.expr()
            for stage in new_model.stages:
                for r in new_model.r:
                    for t in t_per_stage[stage]:
                        for d in new_model.d:
    ...
    for stage in new_model.stages:
        variable_operating_cost.append(
            new_model.Bl[stage].variable_operating_cost.expr()
        )
        ...
        total_energy_generated.append(
            sum(
                new_model.Bl[stage].P[i, r, t, d, s].value
                * new_model.n_d[d].value
                * pow(10, -6)
                for (i, r) in new_model.i_r
                for t in t_per_stage[stage]
                for d in new_model.d
                for s in new_model.hours
            )
        )
        coal_energy_generated.append(
            sum(
                new_model.Bl[stage].P[i, r, t, d, s].value
                * new_model.n_d[d].value
                * pow(10, -6)
                ...
                for d in new_model.d
                for s in new_model.hours
                if i in new_model.co
            )
        )
        natural_gas_energy_generated.append(
            sum(
                new_model.Bl[stage].P[i, r, t, d, s].value
                * new_model.n_d[d].value
                * pow(10, -6)
                ...
                for d in new_model.d
                for s in new_model.hours
                if i in new_model.ng
            )
        )
        nuclear_energy_generated.append(
            sum(
                new_model.Bl[stage].P[i, r, t, d, s].value
                * new_model.n_d[d].value
                * pow(10, -6)
                ...
                for d in new_model.d
                for s in new_model.hours
                if i in new_model.nu
            )
        )
        solar_energy_generated.append(
            sum(
                new_model.Bl[stage].P[i, r, t, d, s].value
                * new_model.n_d[d].value
                * pow(10, -6)
                ...
                for d in new_model.d
                for s in new_model.hours
                if (i in new_model.pv or i in new_model.csp)
            )
        )
        wind_energy_generated.append(
            sum(
                new_model.Bl[stage].P[i, r, t, d, s].value
                * new_model.n_d[d].value
                * pow(10, -6)
                ...
                for d in new_model.d:
                    for s in new_model.hours:
                        ...
                        if new_model.Bl[stage].P_flow[l, t, d, s].value > 0.001:
                            total_power_flow[sr + "_" + er] += (
                                new_model.Bl[stage].P_flow[l, t, d, s].value
                                * new_model.n_d[d].value
                                * pow(10, -6)
                            )
    ...
    operating_related_cost["nuclear_energy_generated"] = np.sum(
        nuclear_energy_generated
    )
    operating_related_cost["coal_energy_generated"] = np.sum(coal_energy_generated)
    operating_related_cost["natural_gas_energy_generated"] = np.sum(
        natural_gas_energy_generated
    )
...
def write_GTEP_results(m, InvestData, config, results=[], ub_problem={}, mode="w"):
    ...
    for stage in m.stages:
        ...
        extending_thermal_generator_cost.append(
            m.Bl[stage].extending_thermal_generator_cost.expr()
        )
        renewable_generator_cost.append(m.Bl[stage].renewable_generator_cost.expr())
        extending_renewable_generator_cost.append(
            m.Bl[stage].extending_renewable_generator_cost.expr()
        )
        ...
        coal_capacity.append(
            sum(
                m.Qg_np[th, r] * m.Bl[stage].ngo_th[th, r, stage].value
                for th, r in m.i_r
                if th in m.co
            )
        )
        natural_gas_capacity.append(
            sum(
                m.Qg_np[th, r] * m.Bl[stage].ngo_th[th, r, stage].value
                for th, r in m.i_r
                if th in m.ng
        ...
        solar_capacity.append(
            sum(
                m.Qg_np[rn, r] * m.Bl[stage].ngo_rn[rn, r, stage].value
                for rn, r in m.i_r
                if (rn in m.pv or rn in m.csp)
            )
        )
        wind_capacity.append(
            sum(
                m.Qg_np[rn, r] * m.Bl[stage].ngo_rn[rn, r, stage].value
                for rn, r in m.i_r
                if rn in m.wi
        ...
        for r in m.r:
            coal_capacity_region[r].append(
                sum(
                    m.Qg_np[th, r] * m.Bl[stage].ngo_th[th, r, stage].value
                    for th, rr in m.i_r
                    if (rr == r and th in m.co)
                )
            )
            natural_gas_capacity_region[r].append(
                sum(
                    m.Qg_np[th, r] * m.Bl[stage].ngo_th[th, r, stage].value
                    for th, rr in m.i_r
                    if (rr == r and th in m.ng)
                )
            )
            nuclear_capacity_region[r].append(
                sum(
                    m.Qg_np[th, r] * m.Bl[stage].ngo_th[th, r, stage].value
                    for th, rr in m.i_r
                    if (rr == r and th in m.nu)
                )
            )
            solar_capacity_region[r].append(
                sum(
                    m.Qg_np[rn, r] * m.Bl[stage].ngo_rn[rn, r, stage].value
                    for rn, rr in m.i_r
                    if (rr == r and (rn in m.pv or rn in m.csp))
                )
            )
            wind_capacity_region[r].append(
                sum(
                    m.Qg_np[rn, r] * m.Bl[stage].ngo_rn[rn, r, stage].value
                    for rn, rr in m.i_r
                    if (rr == r and rn in m.wi)
                )
            )
        total_energy_generated.append(
            sum(
                m.Bl[stage].P[i, r, t, d, s].value * m.n_d[d].value * pow(10, -6)
                for (i, r) in m.i_r
                for t in t_per_stage[stage]
                for d in m.d
                for s in m.hours
                ...
                for d in m.d:
                    for s in m.hours:
                        ...
                        if m.Bl[stage].P_flow[l, t, d, s].value > 0:
                            temp_power_flow[sr][er] += (
                                m.Bl[stage].P_flow[l, t, d, s].value
                                * m.n_d[d].value
                                * pow(10, -6)
                            )
                        else:
                            temp_power_flow[er][sr] -= (
                                m.Bl[stage].P_flow[l, t, d, s].value
                                * m.n_d[d].value
                                * pow(10, -6)
                            )
    ...
    with open(outputfile, mode, newline="") as results_file:
        ...
        for i in range(len(m.stages)):
            new_row = {
                ...
                "extending_renewable_generator_cost": extending_renewable_generator_cost[
                    i
                ],
        ...
        for stage in m.stages:
            for l in m.l_new:
                if m.Bl[stage].ntb[l, stage].value > 0.1:
                    temp_row = [
                        stage,
                        InvestData.tielines[l - 1]["Near Area Name"],
                        InvestData.tielines[l - 1]["Far Area Name"],
                    ]
        ...
        results_writer.writerow(
            [
                "load ",
                m.L["Northeast", last_stage, largest_d, largest_s].value,
                m.L["West", last_stage, largest_d, largest_s].value,
                m.L["Coastal", last_stage, largest_d, largest_s].value,
                m.L["South", last_stage, largest_d, largest_s].value,
                m.L["Panhandle", last_stage, largest_d, largest_s].value,
            ]
        )
        ...
        for r in m.r:
            new_row.append(
                sum(
                    m.Bl[last_stage].P[i, r, last_stage, largest_d, largest_s].value
                    for i, rr in m.i_r
                    if rr == r
                )
            )
        ...
        for r in m.r:
            new_row.append(
                sum(
                    m.Bl[last_stage]
                    .p_charged[j, r, last_stage, largest_d, largest_s]
                    .value
                    for j in m.j
                )
            )
        ...
        for r in m.r:
            new_row.append(
                sum(
                    m.Bl[last_stage]
                    .p_discharged[j, r, last_stage, largest_d, largest_s]
                    .value
                    for j in m.j
                )
            )
        ...
        for r in m.r:
            ...
            for l in m.l:
                ...
                if er == r:
                    temp_P += (
                        m.Bl[last_stage]
                        .P_flow[l, last_stage, largest_d, largest_s]
                        .value
                    )
                if sr == r:
                    temp_P -= (
        ...
        for r in m.r:
            new_row.append(
                m.Bl[last_stage].cu[r, last_stage, largest_d, largest_s].value
            )
        results_writer.writerow(new_row)
        ...
        for l in m.l:
            if (
                abs(m.Bl[last_stage].P_flow[l, last_stage, largest_d, largest_s].value)
                > 0.1
            ):
                results_writer.writerow(
                    [
        ...
        results_writer.writerow([])
        results_writer.writerow(
            ["total_investment_cost", sum(total_investment_cost[:])]
        )
        if results != []:
            if "Solver" in results:
                if "Time" in results["Solver"][0]:
                    results_writer.writerow(
                        ["solver fullspace time", results["Solver"][0]["Time"]]
                    )
                elif "Wallclock time" in results["Solver"][0]:
                    results_writer.writerow(
                        [
                            "solver fullspace time",
                            results["Solver"][0]["Wallclock time"],
                        ]
                    )
            ...
            if "Wallclock time" in results:
                results_writer.writerow(
                    ["nested benders walltime", results["Wallclock time"]]
                )
            ...
            if "upper_bound_obj" in ub_problem:
                results_writer.writerow(
                    ["upper_bound_obj", ub_problem["upper_bound_obj"]]
                )
...
def write_repn_results(operating_cost, config, cluster_results={}):
    ...
    with open(outputfile, "a", newline="") as results_file:
        ...
        fieldnames = ["day_number"] + list(
            operating_cost[list(operating_cost.keys())[0]].keys()
        )
        writer = csv.DictWriter(results_file, fieldnames=fieldnames)
        ...
        average = {
            key: (sum(operating_cost[day][key] for day in days) / len(operating_cost))
