...
for r in regions:
    ...
    with open(
        "for_cluster/" + "CF_" + r + "_PV2012.csv", "w", newline=""
    ) as results_file:
                ...
                writer.writerow(
                    {
                        "time": row,
                        "year": 2012,
                        "ercot": max(0, min(1, PV.iat[i, 0] / np_PV)),
                    }
                )
            i += 1
    ...
    with open(
        "for_cluster/" + "CF_" + r + "_CSP2012.csv", "w", newline=""
    ) as results_file:
        writer = csv.DictWriter(results_file, fieldnames=fieldnames)
        writer.writeheader()
        i = 0
            ...
            writer.writerow(
                {
                    "time": row,
                    "year": 2012,
                    "ercot": max(0, min(1, CSP.iat[i, 0] / np_CSP)),
                }
            )
            i += 1

    wind = pd.read_csv(r + "/" + r + "_windspeed2012.csv", index_col=0, header=0).iloc[
        :, :7
    ]
    with open(
        "for_cluster/" + "CF_" + r + "_wind2012.csv", "w", newline=""
    ) as results_file:
        writer = csv.DictWriter(results_file, fieldnames=fieldnames)
        writer.writeheader()
        i = 0
            ...
            writer.writerow(
                {
                    "time": row,
                    "year": 2012,
                    "ercot": max(0, min(1, power_curve[pos] / np_wind)),
                }
            )
            i += 1
