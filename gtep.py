...
class GTEP:
	...
	def __init__(self, **kwds):
		...
		self.InvestData, self.OperationalData = load_db_data(self.clustering_result, self.config)		
	...
	def add_extremedays(self):
		...
		while True:
			...
			self.InvestData, self.OperationalData = load_db_data(cluster_results, self.config)			
			self.solve_model()
			...
			if iter_ == 1:
				...
				write_GTEP_results(self.m, self.InvestData, self.config, self.opt_results, mode="a")
			...
			for i in range(1, n_stages+1):
				t_per_stage[i] = [i]
			new_model = b.create_model(n_stages, t_per_stage, formulation, self.InvestData, SingleDayData)
			...
			for day in range(1, 366):
				operating_cost[day] = eval_investment_single_day(new_model, day, self.config) 
			...
			self.config.logger.info("%s extreme days have been added there are still %s infeasible days" % (iter_, len(infeasible_days),))
