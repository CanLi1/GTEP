...
def load_db_data(clustering_result, config):
	if config.clustering_algorithm == "kmeans" or config.clustering_algorithm == "kmeans_exact":
		OperationalData.read_representative_days_mean(config.time_horizon, clustering_result['labels'])
	else:
		OperationalData.read_representative_days(config.time_horizon, clustering_result['medoids'], clustering_result['weights'])
