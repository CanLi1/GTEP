...
| algorithm  | Select which algorithm to use. Note that for GTEP model, only "Benders" and "fullspace" are recommended. | "fullspace", "Benders", "NestedBenders", "SDDP"                                                                                         |
| stochastic | Select which parameters are considered as stochastic. Only works for GEP models.        | give a list of parameters. If the list is empty, then solve a deterministic model. Possible input is ["CarbonTax", "Load", "FuelPrice"] |